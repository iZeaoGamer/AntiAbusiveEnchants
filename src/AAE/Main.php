				...
				if($i->hasEnchantments()){
					foreach($i->getEnchantments() as $e){
						if($e->getLevel() >= $max){
							...
							$p->sendMessage(TF::GREEN."[AntiAbusiveEnchants]".TF::BLUE.$i->getName()." has been removed from your inventory for being above or eqaul to the max enchantment level!");
						}
					}
				}
			}
